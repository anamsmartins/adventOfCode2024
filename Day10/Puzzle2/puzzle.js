...
trailheads.forEach((trailhead) => {
  trailheadScores.set(trailhead, { positions: [], rating: 0 });
});
...
function moveRecursively(
  ...
  if (newValue == 9) {
    trailheadScores.get(trailhead).rating++;
    if (
      !trailheadScores
        .get(trailhead)
        .positions.some((arr) =>
      ...
      trailheadScores.get(trailhead).positions.push(newPosition);
      return;
    }
  }
...
for (let finishPositions of trailheadScores.values()) {
  ratings += finishPositions.rating;
}
